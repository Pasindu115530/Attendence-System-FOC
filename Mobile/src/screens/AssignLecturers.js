...
export default function AssignLecturers({ navigation }) {
  const [lecturers, setLecturers] = useState([]);
  const [selectedLecturerIndex, setSelectedLecturerIndex] = useState('');
  ...
  const fetchLecturers = async () => {
    try {
      const res = await post('/get_lecturers', {});

      if (res.status === 'success') {
        setLecturers(res.data.lecturers || []);
        if (res.data.lecturers?.length > 0) {
          setSelectedLecturerIndex(res.data.lecturers[0].user_id);
        }
      }
    } catch (error) {
      console.error(error);
  ...
  const fetchSubjects = async () => {
    if (!selectedLecturerIndex) return;
    setLoading(true);
    try {
      const res = await post('/get_lecturer_subjects', {
        lecturer_index: selectedLecturerIndex
      });
      if (res.status === 'success') {
  ...
  const handleSave = async () => {
    if (!selectedLecturerIndex) {
      showAlert('Error', 'Please select a lecturer.', 'error');
      return;
    }
    ...
    setSaving(true);
    try {
      const res = await post('/assign_lecturer_subject', {
        lecturer_index: selectedLecturerIndex,
        subject_ids: assignedIds
      });

      if (res.status === 'success') {
        showAlert('Success', 'Subjects assigned to lecturer successfully!', 'success');
      } else {
    ...
    <View style={styles.container}>
        ...
        >
          <View style={styles.headerTop}>
            ...
            <View style={styles.headerTitleContainer}>
              <Text style={styles.headerSubtitle}>Staff Configuration</Text>
              <Text style={styles.headerTitle}>Assign Lecturers</Text>
            </View>
            <View style={{ width: 40 }} />
          </View>
        </LinearGradient>
      ...
      <Animated.View style={{ flex: 1, opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
        ...
        <View style={styles.filterCard}>
          <Text style={styles.filterLabel}>Select Lecturer</Text>
          <View style={styles.pickerWrapper}>
            <Picker
              selectedValue={selectedLecturerIndex}
              onValueChange={(val) => setSelectedLecturerIndex(val)}
              style={styles.picker}
            >
              {lecturers.map(l => (
                <Picker.Item key={l.user_id} label={`${l.full_name} (${l.user_id})`} value={l.user_id} />
              ))}
            </Picker>
          </View>
        </View>
            ...
            <View style={[styles.listContainer, { flex: 1 }]}>
              <View style={{ paddingHorizontal: 20, paddingTop: 10 }}>
                ...
                <Text style={styles.sectionDesc}>
                  Select subjects taught by this lecturer
                </Text>
              </View>
